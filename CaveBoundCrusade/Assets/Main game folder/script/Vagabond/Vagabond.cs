...
public class Vagabond : EnemyParentScript, IDataPersistance{
    ...
    public bool stunPending = false;
    private bool canStunAgain = false;
    private float canStunAgainTimer = 10f;
    ...
    [Header("Damage")]
    [SerializeField] protected float _damageNormal = 35f;
    [SerializeField] protected float _damageHeavy = 50f;
    ...
    void Update() {
        ...
        Debug.Log("Damage Taken : " + _damageTaken);
    ...
    void EnemyBehaviour() {
        ...
        if (isCharingAttack || isAttacking || isHeavyAttacking || isStunned || isDashing || isJumping || isHeavyAttacking) return;
        else if (isBlocking){ 
            flip();
            return;
        }
    ...
    void Chase(){
        ...
        targetSpeed = Mathf.Lerp(rigbody.velocity.x, chaseMaxSpeed, 1);
        ...
        rigbody.AddForce(targetSpeed * new Vector2(direction,0), ForceMode2D.Force);
    ...
    void CanStunAgainTimer()
    {
        if (!canStunAgain)
        {
            canStunAgainTimer -= Time.deltaTime;
            if(canStunAgainTimer <= 0)
            {
                canStunAgainTimer = 10f;
                canStunAgain = true;
            }
        }
    }
    ...
    private void TakeDamage(float Damage){
        ...
        if (_damageTaken >= ORIGINAL_HEALTH / 6 && canStunAgain)
            stunPending = true;
    ...
    private void OnTriggerEnter2D(Collider2D collision){
        ...
        else if (collision.gameObject.CompareTag("HitBoxHeavy"))
